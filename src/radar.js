...
export class TacticalRadar {
  constructor(target, options = {}) {
    if (!target) throw new TypeError('TacticalRadar 需要一個 canvas 或容器。');
    ...
    this.canvas.setAttribute('aria-label', '戰術雷達。上方為北。圓形是隊友，菱形是被標記的敵人。');
  ...
  drawObjective(objective) {
    ...
    ctx.fillStyle = colour;
    ctx.font = `700 ${point.edge ? 7 : 8}px system-ui, "PingFang TC", "Noto Sans TC", "Microsoft JhengHei", sans-serif`;
