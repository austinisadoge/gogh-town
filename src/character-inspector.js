...
const MODES = Object.freeze({face: '臉部', full: '全身', walk: '行走', crouch: '蹲下'});
...
export function createCharacterInspector({onClose} = {}) {
    ...
    <div class="ci-shell">
      <header class="ci-header">
        <div class="ci-heading">
          <p class="ci-eyebrow">畫家名冊 <span class="ci-count"></span></p>
          <h2 id="${id}-name"></h2>
          <p class="ci-age"></p>
        </div>
        <div class="ci-artist-nav" aria-label="瀏覽畫家">
          <button class="ci-icon-button ci-previous" type="button" aria-label="上一位畫家" title="上一位畫家">‹</button>
          <button class="ci-icon-button ci-next" type="button" aria-label="下一位畫家" title="下一位畫家">›</button>
        </div>
        <button class="ci-icon-button ci-close" type="button" aria-label="關閉角色工作室" title="關閉 · Esc">×</button>
      </header>
      <div class="ci-stage">
        <button class="ci-reset" type="button" title="回到四分之三視角">↺ <span>重設視角</span></button>
        <p class="ci-status" role="status">正在打開工作室…</p>
        <p class="ci-stage-hint" aria-hidden="true">拖曳旋轉 <span>·</span> 滾輪縮放</p>
      </div>
      <footer class="ci-footer">
        <div class="ci-modes" role="group" aria-label="角色視角">
  ...
  function updateLabels() {
    ...
    ageLabel.textContent = `${member.age1888} 歲 · 1888 年`;
    ...
    if (renderer) renderer.domElement.setAttribute('aria-label', `${member.name}，${MODES[mode]}視角。拖曳或用方向鍵旋轉，加減號縮放，Home 重設。`);
    query('.ci-previous').title = `上一位：${roster[(selectedIndex + roster.length - 1) % roster.length].member.shortName}`;
    query('.ci-next').title = `下一位：${roster[(selectedIndex + 1) % roster.length].member.shortName}`;
  }
  ...
  function chooseArtist(selection) {
    ...
    if (index < 0) throw new RangeError('找不到這位畫家');
    selectedIndex = index;
    updateLabels();
  ...
  function setMode(value) {
    ...
    if (!Object.hasOwn(MODES, normalized)) throw new RangeError(`沒有這個角色視角：${value}`);
    mode = normalized;
  ...
  function open(selection = {}) {
    if (disposed) throw new Error('這個角色工作室已經關掉了');
    ...
    try {
      ...
      statusLabel.textContent = '工作室打不開，關掉再試一次。';
      statusLabel.hidden = false;
      closeButton.focus({preventScroll: true});
    }
    return state();
