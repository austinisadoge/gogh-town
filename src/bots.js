...
// 這些座標是照原始小鎮標好的。投影與路徑用的導航幾何跟玩家操控時完全相同。
export const BOT_SECTORS=Object.freeze([
 {id:'bedroom',name:'臥室',points:[[-1.65,3.5],[-9.7,6.8],[3,14],[16,17]]},
 {id:'cafe',name:'咖啡館露臺',points:[[-14,-4],[-11,-20],[-8,-30],[-18,6]]},
 {id:'rhone',name:'隆河碼頭',points:[[3,-42],[18,-43],[27,-36],[20,-38]]},
 {id:'wheat',name:'麥田',points:[[48,23],[49,4],[69,23],[56,17]]},
 {id:'sunflowers',name:'向日葵',points:[[82,16],[82,-6],[91,1],[77,20]]},
 {id:'crossroads',name:'鄉間小路',points:[[35,-24],[39,-8],[59,-12],[47,-21]]},
]);
...
export class BotDirector {
 ...
 useUtility(actor,s,game,now){
  ...
  if(s.visible&&s.target){
   ...
   if(hasSmoke&&(actor.health<(actor.maxHealth??100)*.5||s.intent==='establish exhibition'||s.intent==='remove beacon')&&d>10){
    const point=actor.position.clone().lerp(s.lastSeen,.42);if(game.requestUtility(actor,'smoke',point)){s.nextUtility=now+11;if(team)team.nextUtilityAt=now+6;this.radio(game,s,actor,'煙霧封住路口。');return;}
   }
   if((actor.utility?.frag||0)>0&&d>12&&d<30){
    ...
    if(!friendlyNear&&this.los(game,eye(actor),chest(s.target))&&game.requestUtility(actor,'frag',s.lastSeen.clone())){s.nextUtility=now+14;if(team)team.nextUtilityAt=now+6;this.radio(game,s,actor,'顏料罐丟向暴露的角度。');}
   }
  }
 }
 ...
 act(actor,s,game,now,dt){
  ...
  if(interact){
   ...
   if(Math.hypot(actor.velocity?.x||0,actor.velocity?.z||0)<.2){game.requestInteract(actor,dt);this.radio(game,s,actor,game.tactical.planted?'正在拆掉色彩信標。':'正在佈展。');}
