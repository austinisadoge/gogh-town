...
const teamName=t=>FACTIONS[t]?.shortName||'平手';
...
let storedSettings={};try{storedSettings=JSON.parse(localStorage.getItem('gogh-town-settings')||'{}');}catch{}
...
function newActor(team,i){
 ...
 const a={id:team*6+i,name:isPlayer?'你':identity.shortName,artistName:identity.name,identity,team,role:identity.role,personality:identity.personality,isPlayer,
...
function applySettings(next){Object.assign(settings,next);settings.volume=clamp(Number(settings.volume)||0,0,1);settings.sensitivity=clamp(Number(settings.sensitivity)||1,.25,2.5);try{localStorage.setItem('gogh-town-settings',JSON.stringify(settings));}catch{}audio.setVolume(settings.volume);const q=settings.graphics;town.setQuality(q==='full'?1:q==='performance'?.48:.73);renderer.setPixelRatio(Math.min(devicePixelRatio,q==='full'?1.7:q==='performance'?1:1.25));renderer.setSize(innerWidth,innerHeight);}
...
function damageActor(victim,amount,attacker,headshot=false,meta={}){
 ...
 if(victim.isPlayer){view.cancelSpray?.();audio.hurt();const delta=attacker?attacker.position.clone().sub(victim.position):V();ui.showDamage(Math.atan2(delta.x,delta.z)-victim.yaw);killRecap={killer:attacker?.artistName||attacker?.name||'爆炸',portrait:attacker?.portrait,team:attacker?.team,weapon:WEAPON_BY_ID[meta.kind]?.name||meta.kind||'Impact',damage:Math.round(victim.damageContributors.get(attacker?.id)?.damage||actual),hits:victim.damageHits.get(attacker?.id)||1,distance:Math.round(meta.distance||delta.length()),headshot};}
...
function requestSpray(a=player,direction){
 ...
 if(!result.ok){if(a.isPlayer&&result.reason==='wall'){sprayFeedback='對準附近的牆';sprayFeedbackUntil=time+1.2;ui.showToast(sprayFeedback,1200);}return false;}
...
function finishMatch(){if(phase==='results')return;phase='results';paused=false;clearInput();releaseLock();view.setVisible(false);audio.end(rules.winner===player.team);ui.showResults({winner:rules.winner,playerTeam:player.team,scores:rules.scores,mode:config.mode,actors,player,reason:rules.roundHistory?.at(-1)?.reason||'戰局結束',duration:time,roundHistory:rules.roundHistory||[],mvp:rules.mvp});updateHUD();}
