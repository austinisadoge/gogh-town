...
export const GEAR = Object.freeze([
  {id:'armor',name:'防護背心',description:'把護甲補滿到 100。可以減少子彈傷害，但精準彈的穿透力比較強。',cost:650,kind:'armor'},
  {id:'smoke',name:'顏料煙霧',description:'持續 14 秒的煙幕，擋掉敵人的視線。',cost:300,kind:'utility',max:2},
  {id:'frag',name:'破片顏料罐',description:'定時的範圍傷害。石牆可以擋住爆炸。',cost:350,kind:'utility',max:1},
  {id:'medkit',name:'野戰急救包',description:'站著不動按住 H 兩秒半，最多回復 45 點血。',cost:300,kind:'utility',max:1},
]);
...
export function buy(actor,itemId,rules){
 const item=shopFor(actor,rules).find(i=>i.id===itemId);if(!item?.available)return {ok:false,message:rules?.phase!=='staging'?'只有回合之間才能補給。':'已經裝備了，或是點數不夠。'};
 if(!rules.spendCredits(actor,item.cost))return {ok:false,message:'點數不夠。'};
 if(item.kind==='weapon'){
...
export function changeAttachments(actor,next,rules,mode){
 ...
 for(const key of ['optic','barrel'])if(next[key]!==undefined&&next[key]!==current[key]){
  ...
  if(!allowed.includes(next[key]))return {ok:false,message:'這把武器裝不上那個配件。'};
  result[key]=next[key];cost+=ATTACHMENTS[next[key]]||0;
 }
 if(mode==='tactical'&&(rules.phase!=='staging'||!rules.spendCredits(actor,cost)))return {ok:false,message:'整備階段換配件需要點數。'};
 actor.attachments[actor.weaponId]=result;return {ok:true,message:'武器設定已更新。'};
}
