...
const ABILITIES=[
 {id:'smoke',key:'Q',name:'煙霧',path:'M7 18h11a4 4 0 0 0 .5-8 6 6 0 0 0-11.6-1.5A4.8 4.8 0 0 0 7 18Z'},
 {id:'frag',key:'G',name:'顏料彈',path:'m12 3 2.5 5 5.5-1.5-2 5.5 4 4-6 .5-2 5-3.5-4.5L5 19l1.5-5L2 11l6-2Z'},
 {id:'medkit',key:'H',name:'治療',path:'M9 3h6v6h6v6h-6v6H9v-6H3V9h6Z'},
];
const COMBAT_ICONS={
 frag:{label:'顏料罐',path:ABILITIES.find(a=>a.id==='frag').path},
 melee:{label:'近戰',path:'M5 10V5a2 2 0 0 1 4 0V4a2 2 0 0 1 4 0v1a2 2 0 0 1 4 0v2a2 2 0 0 1 4 0v8l-4 6H8l-5-7a2 2 0 0 1 2-3l3 3v-4Z'},
 unknown:{label:'擊殺',path:'m12 3 9 9-9 9-9-9Zm0 5-4 4 4 4 4-4Z'},
};
...
function portrait(team,role,name='畫家',card=false){
...
export class GameUI{
 ...
 syncConfig(){
  ...
  text('selected-artist-name',artist.shortName);text('selected-weapon',weapon.name);text('selected-weapon-class',(WEAPON_CLASS[weapon.id]||weapon.class).toUpperCase());text('selected-magazine',`${weapon.magSize} 發`);text('selected-fire-type',weapon.automatic?'全自動':weapon.id==='sniper'?'手動槍機':weapon.id==='shotgun'?'泵動':'半自動');
  ...
  const tag=getArtistTag(artist);$('selected-art-tag').src=tagDataURI(artist);$('selected-art-tag').alt=`${artist.shortName} 的塗鴉：${tag.name}`;text('selected-tag-name',tag.name);$('selected-tag-name').title=tag.name;
 ...
 beginIntro(config={}){
  ...
  $('intro-yellow-crest').innerHTML=teamLogoSVG(0);$('intro-independent-crest').innerHTML=teamLogoSVG(1);$('intro-crews').dataset.playerTeam=team;$('intro-crews').setAttribute('aria-label',`${FACTIONS[0].name} 對上 ${FACTIONS[1].name}`);$('scoreboard-panel').hidden=true;
 ...
 updateIntro({active=true,beat=3,progress=0,exiting=false}={}){
  ...
  const next=exiting?'GO':String(Math.max(1,Math.min(3,Number(beat)||3)));if(this.introBeat!==next){this.introBeat=next;const numeral=node('span','intro-number',next);numeral.dataset.echo=next;$('intro-count').replaceChildren(numeral);$('match-intro').setAttribute('aria-label',exiting?'開打':`戰局 ${next} 秒後開始`);}
 ...
 update(state={}){
  ...
  text('friendly-team-name',FACTIONS[team].shortName);text('enemy-team-name',FACTIONS[enemy].shortName);text('friendly-score',state.scores?.[team]||0);text('enemy-score',state.scores?.[enemy]||0);text('match-timer',clock(state.timeRemaining));text('score-target',`先拿 ${state.scoreLimit||20} 分`);
  ...
  text('health-value',displayHealth);$('health-fill').style.width=`${clamp(health/maxHealth)*100}%`;const healthTrack=document.querySelector('.health-track');healthTrack.setAttribute('aria-valuenow',String(displayHealth));healthTrack.setAttribute('aria-valuemax','100');healthTrack.setAttribute('aria-valuetext',`血量 ${displayHealth}／100`);document.querySelector('.health-panel').classList.toggle('low',health<=maxHealth*.3&&alive);
  ...
  const reloading=Boolean(weapon.reloading)&&alive;$('reload-state').classList.toggle('reloading',reloading);text('reload-label',reloading?'換彈中…':'換彈');$('reload-fill').style.width=`${clamp(weapon.reloadProgress)*100}%`;
  ...
  for(const ability of ABILITIES){const el=document.querySelector(`[data-ability="${ability.id}"]`),count=Math.max(0,Number(utility[ability.id])||0),cooldown=Math.max(0,Number(cooldowns[ability.id])||0);el.classList.toggle('empty',count===0);el.classList.toggle('cooling',cooldown>0);el.querySelector('.ability-charge').textContent=String(count);el.querySelector('.ability-state').textContent=cooldown>0?`${Math.ceil(cooldown)}秒`:count===0?'用完':'';el.setAttribute('aria-label',`${ability.key}：${ability.name}，剩 ${count} 次${cooldown?`，${Math.ceil(cooldown)} 秒後可用`:''}`);}
  const healing=alive&&Number.isFinite(state.healing);$('interaction-hud').classList.toggle('show',healing);if(healing){text('interaction-label','治療中 — 按住 H');$('interaction-fill').style.width=`${clamp(state.healing)*100}%`;}
  const sprayReadyIn=Math.max(0,Number(state.spray?.readyIn)||0),canSpray=alive&&sprayReadyIn===0&&state.spray?.canPlace===true;text('spray-status',sprayReadyIn>0?`${Math.ceil(sprayReadyIn)}秒`:'噴漆');$('spray-hud').classList.toggle('cooling',sprayReadyIn>0);$('spray-hud').classList.toggle('can-place',canSpray);$('spray-hud').setAttribute('aria-label',sprayReadyIn>0?`塗鴉 ${Math.ceil(sprayReadyIn)} 秒後可用`:canSpray?'T：在這面牆噴上塗鴉':'T：對準附近的牆才噴得上去');
  ...
  text('taunt-status',tauntActive?'取消':tauntReadyIn>0?`${Math.ceil(tauntReadyIn)}秒`:'嗆聲');$('taunt-hud').classList.toggle('active',tauntActive);$('taunt-hud').classList.toggle('unavailable',!tauntActive&&(!alive||taunt.available===false||tauntReadyIn>0));$('taunt-hud').setAttribute('aria-label',tauntActive?`J：取消${tauntName}`:tauntReadyIn>0?`${tauntName} ${Math.ceil(tauntReadyIn)} 秒後可用`:`J：${tauntName}`);$('taunt-hud').title=tauntActive?`J：取消${tauntName}`:tauntName;
 ...
 findActor(name,team,id){const actors=this.state.actors||[];return actors.find(a=>(id!==undefined&&a.id===id)||(a.team===team&&(a.name===name||a.artistName===name||a.identity?.name===name||(name==='你'&&a.isPlayer))));}
 addKill(event={}){
  ...
  const left=node('span',killerTeam===own?'friendly':'enemy');left.append(portrait(killerTeam,ka?.role,ka?.shortName||event.killer),node('b','',ka?.shortName||event.killer||'顏料'));
  const right=node('span',victimTeam===own?'friendly':'enemy');right.append(portrait(victimTeam,va?.role,va?.shortName||event.victim),node('b','',va?.shortName||event.victim||'畫家'));
 ...
 showHit({headshot=false,killed=false}={}){
  ...
  // 傷害結算已經給了被擊殺者和獎章，武器的命中回呼緊接著進來時要保留原值。
  if(killed){if(!this.eliminationNotice||performance.now()-this.eliminationNotice.time>200)this.showElimination({headshot});return;}
  if(headshot&&!cue.classList.contains('frag-confirmation')){cue.replaceChildren(node('b','','◆'),node('span','','爆頭'));cue.classList.add('active');this.timed('confirmation',450,()=>this.clearElimination());}
 }
 clearElimination(){
 ...
 showElimination({victim='',portrait:portraitPath='',healthGain=0,healthMax=this.state.player?.maxHealth||150,streak=1,multi=1,headshot=false}={}){
  ...
  if(headshot){const accent=node('i','frag-headshot');accent.title='爆頭';accent.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="5"/><path d="M12 2v5m0 10v5M2 12h5m10 0h5"/></svg>';seal.append(accent);}
  const label=count>1?(count===2?'雙殺':count===3?'三殺':`${count} 連殺`):headshot?'爆頭':'';
  ...
  else identity.append(node('b','frag-name','好球'));
  details.append(identity);
  const displayGain=Math.round(clamp(healthGain/Math.max(1,Number(healthMax)||150))*100);
  if(displayGain>0){const heal=node('small','frag-heal',`回血 +${displayGain}`);details.append(heal);}
 ...
 showSprayFeedback(result){
  ...
  if(!success){const message=typeof result==='string'?result:result?.message;this.showToast(message||'要對準牆面',900);return;}
 ...
 renderKiller(killer,recap){const identity=killer?this.identity(killer):null,name=identity?.shortName||recap?.killer||'某位對手',signature=`${killer?.team}-${identity?.role}-${name}`;if(signature===this.killerSignature)return;this.killerSignature=signature;text('killer-name',name);$('killer-avatar').replaceChildren(portrait(killer?.team,identity?.role,name));}
 ...
 renderScoreboard(){
  ...
  for(const team of [own,1-own]){const section=node('section',`scoreboard-team ${team===own?'friendly':'enemy'}`),header=node('header');header.append(crest(team),node('b','',`${team===own?'我方':'敵方'} · ${FACTIONS[team].shortName}`),node('strong','',this.state.scores?.[team]||0));section.append(header);
   const table=node('table','score-table'),thead=node('thead'),tr=node('tr');for(const label of ['畫家','擊殺','陣亡'])tr.append(node('th','',label));thead.append(tr);table.append(thead);const tbody=node('tbody');
   actors.filter(a=>a.team===team).sort((a,b)=>(b.kills||0)-(a.kills||0)||(a.deaths||0)-(b.deaths||0)).forEach(actor=>{const identity=this.identity(actor),row=node('tr',`${actor.isPlayer?'you':''} ${actor.alive===false?'dead':''}`),name=node('td'),label=node('span','',identity.shortName);name.append(portrait(team,identity.role,identity.shortName),label);if(actor.isPlayer)label.append(node('small','','你'));row.append(name,node('td','tags',actor.kills||0),node('td','',actor.deaths||0));tbody.append(row);});table.append(tbody);section.append(table);parent.append(section);
  }
 }
 showResults(result={}){
  this.endIntro();this.clearElimination();
  ...
  text('result-title',winner===null||winner===undefined?'平手。':winner===own?'我方獲勝。':'我方落敗。');document.querySelector('.results-card').classList.toggle('defeat',winner!==null&&winner!==undefined&&winner!==own);
  $('result-friendly-crest').innerHTML=teamLogoSVG(own);$('result-enemy-crest').innerHTML=teamLogoSVG(enemy);
  text('result-friendly-name',FACTIONS[own].shortName);text('result-enemy-name',FACTIONS[enemy].shortName);text('result-friendly-score',result.scores?.[own]??0);text('result-enemy-score',result.scores?.[enemy]??0);text('result-artist',identity.shortName);text('result-performance',`擊殺 ${player.kills||0} · 陣亡 ${player.deaths||0}`);$('result-avatar').replaceChildren(portrait(own,identity.role,identity.shortName));$('pointer-resume').classList.remove('show');$('rematch-button').focus({preventScroll:true});
 }
 ...
 // 保留給舊版玩法的呼叫點；顏料對決沒有整備階段，也沒有下單流程。
